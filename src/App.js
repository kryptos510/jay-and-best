...
function fbSet(path, val) {
  set(ref(db, path), val).catch(console.error);
}
...
.sync-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(255,255,255,.15);border-radius:20px;padding:3px 10px;font-size:11px;color:rgba(255,255,255,.85);margin-top:8px;position:relative;z-index:1;}
.sync-dot{width:6px;height:6px;border-radius:50%;background:#7dff9a;}
.sync-dot.off{background:#ff9a7d;}
...
.cal-day.has-event::after{content:'♥';position:absolute;bottom:0;right:1px;font-size:7px;color:var(--rose);}
...
.loading{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;background:var(--cream);gap:12px;}
.loading-title{font-family:'Cormorant Garamond',serif;color:var(--rose);font-size:22px;font-style:italic;}
.loading-sub{font-size:13px;color:var(--ink-s);}
`;
...
export default function App() {
  ...
  // Shared state — all come from Firebase
  ...
  useEffect(() => {
    let ready = { checked: false, customItems: false, calEvents: false, notes: false };
    const checkReady = () => {
      if (Object.values(ready).every(Boolean)) setSynced(true);
    };

    const u1 = fbListen("checked", v => { setChecked(v || {}); ready.checked = true; checkReady(); });
    const u2 = fbListen("customItems", v => { setCustomItems(v ? Object.values(v) : []); ready.customItems = true; checkReady(); });
    const u3 = fbListen("calEvents", v => { setCalEvents(v || {}); ready.calEvents = true; checkReady(); });
    const u4 = fbListen("notes", v => { setNotes(v ? Object.values(v).sort((a,b) => b.id - a.id) : []); ready.notes = true; checkReady(); });
  ...
  const addCustomItem = () => {
    ...
    const obj = {};
    next.forEach(i => { obj[i.id] = i; });
    fbSet("customItems", obj);
  };
  ...
  const addNote = () => {
    ...
    const id = Date.now();
    const note = { id, who, color: noteColor, text: noteText.trim(), date: new Date().toISOString() };
    setNoteText("");
    fbSet(`notes/${id}`, note);
  };
  ...
  if (!synced) return (
    <>
      ...
      <div className="loading">
        ...
        <div className="loading-title">Jay &amp; Best</div>
        <div className="loading-sub">Connecting our little world…</div>
      </div>
    </>
  );
      ...
      <div className="app">
        ...
        <div className="hdr">
          ...
          <div className="sync-badge">
            <div className="sync-dot"/> Live sync on
          </div>
        </div>
