...
export default function JayAndBest() {
  ...
  useEffect(() => {
    (async () => {
      const c = await sget(KEYS.checklist); if (c) setChecked(c);
      const ci = await sget(KEYS.customItems); if (ci) setCustomItems(ci);
      const cal = await sget(KEYS.calendar); if (cal) setCalEvents(cal);
      const n = await sget(KEYS.notes); if (n) setNotes(n);
      setLoaded(true);
    })();
  }, []);
  ...
  const toggleItem = async (id) => {
    ...
    setChecked(next); await sset(KEYS.checklist, next);
  };
  const addCustomItem = async () => {
    ...
    setCustomItems(next); setNewItem(""); await sset(KEYS.customItems, next);
  };
  ...
  const saveDay = async () => {
    ...
    setCalEvents(next); setSelectedDay(null); setDayInput(""); await sset(KEYS.calendar, next);
  };
  const delCalEvent = async (key) => {
    const next = { ...calEvents }; delete next[key];
    setCalEvents(next); await sset(KEYS.calendar, next);
  };
  ...
  const addNote = async () => {
    ...
    setNotes(next); setNoteText(""); await sset(KEYS.notes, next);
  };
  const delNote = async (id) => {
    const next = notes.filter(n => n.id !== id);
    setNotes(next); await sset(KEYS.notes, next);
  };

  if (!loaded) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",background:"#fdf8f4",fontFamily:"'Cormorant Garamond',serif",color:"#c4756a",fontSize:20,fontStyle:"italic"}}>
      Loading Jay &amp; Best…
    </div>
  );
